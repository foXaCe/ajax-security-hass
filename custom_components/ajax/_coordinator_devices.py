...
* `_normalize_device_attributes`: maps the raw Ajax field names to the
  shapes our handlers expect (battery / signal / tamper / temperature /
  malfunctions / device-specific extras).
...
* `_reset_expired_motion_detections`: clears the motion_detected impulse
  state after the 30 s no-event window.
...
class AjaxDevicesMixin:
    """Coordinator mixin: device polling + attribute normalisation + cleanup."""
